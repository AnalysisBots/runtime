...
## This is copied from https://github.com/RepoGrams/RepoGrams/blob/master/app/git_helpers.py
## The file has gone through some minor modification to fit the current system
## but it still need to be adapted to the current system
...
class GitHelper(object):
    def __init__(self, repo_url, dir_manager, credentials_manager):
        ...
        if not os.path.isfile(dir_path + '/HEAD'):
            try:
                self.repo = pygit2.clone_repository(repo_url, dir_path, credentials=my_credentials)
